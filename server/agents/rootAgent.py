...
#             "itinerary": {},
#             "weather_data": {},            # ADD THIS
#             "is_weather_response": False  
#         }
...
def planning_agent_node(state: TravelAgentState) -> TravelAgentState:
    ...
    sub_state = {
        ...
        "tool_names": [],
        "last_tool_call_ids": [],
        "weather_data": state.get("weather_data", {})  # Add weather data to state
    }
    ...
    if weather_data:
        for location, data in weather_data.items():
            if "report" in data:
                weather_info = f"\nCurrent weather information for {location}: {data['report']}"
                sub_state["messages"].append(SystemMessage(content=weather_info))
                logger.info(f"Added weather information for {location} to planning agent input")
    
    # Invoke the planning agent with the enhanced sub_state
    sub_result = planning_agent_instance.graph.invoke(sub_state)
    ...
    for msg in reversed(sub_result["messages"]):
        if isinstance(msg, AIMessage):
            ...
            if tool_data:
                ...
                # Add weather data to the API data if not already included
                if weather_data and "api_data" in output and "weather" not in output["api_data"]:
                    output["api_data"]["weather"] = weather_data
...
class Agent:
    ...
    async def ainvoke(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
                ...
                if isinstance(output, str):
                    try:
                        ...
                        if output.strip().startswith('{') and output.strip().endswith('}'):
                            ...
                            # Look for all data types
                            for key in ["restaurants", "flights", "flight", "hotels", "attractions", "weather"]:
                                if key in parsed:
                                    # Standardize key names (flight -> flights)
                                    data_key = "flights" if key == "flight" else key
                                    print(f"Found {key} data with {len(parsed[key]) if isinstance(parsed[key], list) else 'non-list'} items")
                                    response[data_key] = parsed[key]
                                    if data_key not in response["api_data"]:
                                        response["api_data"][data_key] = parsed[key]
                    except json.JSONDecodeError as e:
                    ...
                    # Even if JSON parsing failed, look for embedded data
                    if "restaurants" in output or "flights" in output or "hotels" in output or "attractions" in output or "weather" in output:
                        print("Found data references in planning agent output string")
                        # Use regex to find data sections
                        import re
        ...
        for msg in reversed(final_state.get("messages", [])):
            ...
            if isinstance(msg, dict) and "content" in msg:
                content = msg["content"]
                if isinstance(content, str) and any(key in content for key in ["restaurants", "flights", "hotels", "attractions", "weather"]):
                    print("Found potential data in message content")
                    # Use same regex approach as above to extract data
                    import re
        ...
        # Add weather data to response if available and not already included
        if final_state.get("weather_data") and "weather" not in response["api_data"]:
            response["api_data"]["weather"] = final_state["weather_data"]
            print("Added weather data from state to response")
        ...
        # Ensure all data is in api_data (final safety check)
        for key in ["restaurants", "flights", "hotels", "attractions", "weather"]:
            if key in response and key not in response["api_data"]:
                print(f"Final safety check: Copying {key} data to api_data")
                response["api_data"][key] = response[key]
        ...
        if "api_data" in response and response["api_data"]:
            for key, value in response["api_data"].items():
                if isinstance(value, list):
                    ...
                    print(f"api_data['{key}'] is a dictionary with {len(value)} keys")
